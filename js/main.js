...
if (typeof prefs.bookmarks == "undefined") {
  chrome.topSites.get(function(topSites) {
    ...
    $.each(settings.contentTypes.bookmarks.default, function(index, bookmark) {
      ...
      let domainMatched =
        domainMatchings[
          getDomainFromUrl(topSites[index].url).replace(/^www\./, "")
        ];
...
$(document).ready(function() {
  ...
  (function() {
    ...
    $.each(domainMatchings, function(dmKey, dmVal) {
      if (typeof dmVal.icon != "undefined") {
        if (typeof iconList[dmVal.icon] == "undefined") {
          // only include the icon once
  ...
  $('[data-content-type]:not([type="checkbox"])').on("focus click", function(
    e
  ) {
    let el = $(this);
    let contentType = el.attr("data-content-type");
  ...
  $('[data-content-type]:not([type="checkbox"])').on("keydown", function(e) {
    ...
    if (
      settings.contentTypes[contentType].allowNewlines === false &&
      e.keyCode === 13
    ) {
  ...
  $('[data-content-type]:not([type="checkbox"])').on("drop paste", function(e) {
    ...
    if (
      settings.contentTypes[contentType].maxCharacters <=
...
function confirmResetSettings() {
  let r = confirm(
    "Are you sure you want to reset all your Minimalite settings?"
  );
  if (r == true) {
    resetSettings();
  }
  ...
  if (key == "bookmarks") {
    ...
    $.each(prefs[key], function(index, bookmark) {
      let newBookmark = $("<a>")
        ...
        .keydown(function(e) {
          if (e.keyCode == 13 || e.keyCode == 32) {
            // if enter or space pressed, trigger link
      ...
      if (typeof bookmark.icon != "undefined") {
        newBookmark.append($("<img>").attr("src", bookmark.icon));
      } else {
...
function getAndUpdateWeather(location, isUsingWeatherId) {
  ...
  $.get("http://api.k0shy.com/weather/?" + param + "=" + location, function(
    data
  ) {
    ...
    .fail(function(e) {
      ...
      $(".weather_location_container")
        .stop()
        .effect("highlight", { color: "ff9595" }, 2000);
    })
    .always(function() {
      $(".weather .loader")
        .stop()
...
function updateWeatherDisplay() {
  if (weather.weather_location != "") {
    ...
    let forecastUrl =
      ...
      "#name=this%20week&units=" +
      fioUnits +
      "&lat=" +
      weather.lat +
      "&lon=" +
      weather.lon; // generate a random number to allow refreshing
